...
import { NavController, NavParams, LoadingController, ToastController, ActionSheetController, AlertController,
...
firebase.firestore().collection("users").doc(uid).set({
  ...
  console.log("token save to the firestore")
}).catch(err => {
  console.log(err);
})
...
getPosts(){
  ...
  loading.present();
  let query = firebase.firestore().collection("posts").orderBy("created", "desc" ).limit
  (this.pageSize);
     ...
     if(change.type == "added"){
      ...
      if (change.type == "modified"){
       for(let i = 0; i < this.posts.length; i++) {
        ...
        console.log("Document with id " + "has been modified.");
      }
...
logout(){
  firebase.auth().signOut().then(() => {
    let toast = this.toastCtrl.create({
      message: "Yon have been logged out",
      duration: 3000
    }).present();
...
this.http.post("https://us-central1-exposureapp-9e773.cloudfunctions.net/updateLikesCount", JSON.stringify(body), {
  responseType: "text"
}).subscribe((data) => {
  console.log(data)

toast.setMessage("Score updated!");
setTimeout(() => {
  toast.dismiss();
}, 3000)
